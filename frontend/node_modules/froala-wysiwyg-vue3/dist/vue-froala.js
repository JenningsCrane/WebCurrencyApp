...
function ownKeys(target) {
    track(target, "iterate" /* TrackOpTypes.ITERATE */, Object(shared_esm_bundler["o" /* isArray */])(target) ? 'length' : ITERATE_KEY);
...
function markRaw(value) {
    Object(shared_esm_bundler["g" /* def */])(value, "__v_skip" /* ReactiveFlags.SKIP */, true);
    return value;
}
...
class ComputedRefImpl {
    constructor(getter, _setter, isReadonly, isSSR) {
        ...
        this.dep = undefined;
        this.__v_isRef = true;
        this[reactivity_esm_bundler_a] = false;
        this._dirty = true;
...
reactivity_esm_bundler_a = "__v_isReadonly" /* ReactiveFlags.IS_READONLY */;
...
function devtoolsInitApp(app, version) {
    runtime_core_esm_bundler_emit("app:init" /* DevtoolsHooks.APP_INIT */, app, version, {
        Fragment,
        Text,
        Comment,
        Static
...
function devtoolsUnmountApp(app) {
    runtime_core_esm_bundler_emit("app:unmount" /* DevtoolsHooks.APP_UNMOUNT */, app);
}
...
function devtoolsComponentEmit(component, event, params) {
    runtime_core_esm_bundler_emit("component:emit" /* DevtoolsHooks.COMPONENT_EMIT */, component.appContext.app, component, event, params);
}
...
function traverse(value, seen) {
    if (!Object(shared_esm_bundler["w" /* isObject */])(value) || value["__v_skip" /* ReactiveFlags.SKIP */]) {
        return value;
    }
    seen = seen || new Set();
    if (seen.has(value)) {
...
function toHandlers(obj, preserveCaseIfNecessary) {
    ...
    for (const key in obj) {
        ret[preserveCaseIfNecessary && /[A-Z]/.test(key)
            ? `on:${key}`
            : Object(shared_esm_bundler["M" /* toHandlerKey */])(key)] = obj[key];
    }
    return ret;
}
...
function applyOptions(instance) {
    ...
    if (options.beforeCreate) {
        runtime_core_esm_bundler_callHook(options.beforeCreate, instance, "bc" /* LifecycleHooks.BEFORE_CREATE */);
    }
    const { 
    // state
    ...
    if (created) {
        runtime_core_esm_bundler_callHook(created, instance, "c" /* LifecycleHooks.CREATED */);
    }
...
function setupStatefulComponent(instance, isSSR) {
    ...
    if (setup) {
        ...
        setCurrentInstance(instance);
        pauseTracking();
        const setupResult = callWithErrorHandling(setup, instance, 0 /* ErrorCodes.SETUP_FUNCTION */, [( false) ? undefined : instance.props, setupContext]);
        resetTracking();
        unsetCurrentInstance();
